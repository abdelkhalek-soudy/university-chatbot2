...
def transcribe_audio(path):
    try:
        ...
        if file_size < 10:  # أقل من 10 bytes يعتبر فارغ تقريباً
        ...
        for i, attempt in enumerate(attempts):
            try:
                print(f"[INFO] Transcription attempt {i+1}/3 with language: {attempt.get('language', 'auto')}")
                ...
                with open(path, "rb") as f:
                    ...
                    print(f"[INFO] Sending request to OpenAI Whisper...")
                ...
                last_error = str(e)
                print(f"[WARN] Transcription attempt {i+1} failed: {e}")
                continue
        ...
        error_msg = f"[ERROR] All transcription attempts failed. Last error: {last_error}"
        print(error_msg)
        return error_msg
...
def login():
    ...
    if username == ADMIN_USERNAME and password == ADMIN_PASSWORD:
        claims = {"role": "admin"}
        token = create_access_token(identity=username, additional_claims=claims, expires_delta=datetime.timedelta(hours=8))
        return jsonify(access_token=token, student_name=username, role="admin")
    ...
    try:
        ...
        return jsonify(access_token=token, student_name=name, role=claims["role"])
    except Exception as e:
        print("[ERROR] login:", e)
...
def chat():
    ...
    try:
        ...
        try:
            ...
            if current_user:
                ...
                username = "مجهول"
                print("[DEBUG] No JWT identity found - user not logged in")
        except Exception as jwt_e:
            print(f"[DEBUG] JWT extraction failed: {jwt_e}")
            username = "مجهول"
        cur.execute("INSERT INTO logs (message, type, username, timestamp) VALUES (?, 'text', ?, datetime('now', 'localtime'))", (data["message"], username))
        conn.commit()
    except Exception as e:
        print("[WARN] failed to log chat message:", e)
    finally:
        try:
            conn.close()
        except Exception:
    ...
    reply = ask_gpt(data["message"], username)
